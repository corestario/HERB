resource "null_resource" "clickhouse_deploy" {
  ...
  provisioner "local-exec" {
    command = <<-EOA
    echo "${templatefile("${path.module}/templates/ansible_inventory.yml.tpl", { domain = var.domain, testnet_nodes = digitalocean_droplet.testnet[*], env_name = var.env_name, testnet_prometheus_port = var.testnet_prometheus_port, testnet_clients_amount = var.testnet_clients_amount, testnet_client_password = var.testnet_client_password, herb_threshold_1 = var.herb_threshold_1, herb_threshold_2 = var.herb_threshold_2 })}" > ${var.ansible_workdir}/hosts.yml
    EOA
  }
}
